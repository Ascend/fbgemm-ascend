...
def env_setup_path() -> str:
    ...
    for c in candidates:
        if os.path.isfile(c) and _package_has_opp(os.path.dirname(c)):
            return c
    raise FileNotFoundError(f"fbgemm_ascend: env_setup.sh with packaged opp/*/vendors not found (tried: {candidates})")
        ...
        for filename in lib_names:
            ...
            if os.path.isfile(lib_path):
                try:
                    ...
                    if not no_throw:
                        raise
    msg = "fbgemm_ascend: library not found (variant=%s, dirs=%s, candidates=%s)" % (variant, search_dirs, lib_names)
    if no_throw:
        logging.warning(msg)
        return
