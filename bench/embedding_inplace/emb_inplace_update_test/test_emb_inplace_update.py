...
WEIGHT_TY_LIST = [
    SPARSE_TYPE_FP32, SPARSE_TYPE_FP16, SPARSE_TYPE_BF16,
    SPARSE_TYPE_INT8, SPARSE_TYPE_INT4, SPARSE_TYPE_INT2, SPARSE_TYPE_FP8,
]
...
def emb_inplace_update_cpu_reference(
    dev_weights, weights_placements, weights_offsets, weights_tys,
    D_offsets, update_weights, update_table_indices, update_row_indices,
    update_offsets, row_alignment,
):
...
def generate_test_data(T, D_range, rows_range, N, weight_ty_list, row_alignment,
...
def run_test(data):
    ...
    dev_ref = emb_inplace_update_cpu_reference(
        data["dev_weights"], data["weights_placements"], data["weights_offsets"],
        data["weights_tys"], data["D_offsets"], data["update_weights"],
        data["update_table_indices"], data["update_row_indices"],
        data["update_offsets"], data["row_alignment"],
    )
    ...
    torch.ops.fbgemm.emb_inplace_update(
        dev_w_npu, uvm_w_npu, placements_npu, offsets_npu, tys_npu,
        d_offsets_npu, upd_w_npu, upd_t_npu, upd_r_npu, upd_off_npu,
        data["row_alignment"],
    )
    ...
    assert torch.equal(dev_w_result, dev_ref), (
        ...
        f"  diff positions: {torch.nonzero(dev_w_result != dev_ref).flatten().tolist()[:20]}"
    )
...
def test_all_sparse_types(weight_ty):
    ...
    data = generate_test_data(
        T=2, D_range=(D_min, 32), rows_range=(10, 50), N=20,
        weight_ty_list=[weight_ty], row_alignment=16,
    )
    run_test(data)
...
def test_row_alignment_variants(row_alignment):
    ...
    data = generate_test_data(
        T=4, D_range=(4, 32), rows_range=(10, 50), N=30,
...
def test_row_idx_dtype(row_idx_dtype):
    ...
    set_seed(500)
    data = generate_test_data(
        T=3, D_range=(8, 16), rows_range=(10, 30), N=15,
        weight_ty_list=[SPARSE_TYPE_FP32], row_alignment=16,
...
def test_multi_table_mixed_dtype():
    ...
    set_seed(102)
    data = generate_test_data(
        T=5, D_range=(4, 64), rows_range=(50, 200), N=100,
        weight_ty_list=[
            SPARSE_TYPE_FP32, SPARSE_TYPE_FP16, SPARSE_TYPE_INT8,
            SPARSE_TYPE_INT4, SPARSE_TYPE_INT2,
        ],
        row_alignment=16,
    )
    run_test(data)
...
def test_empty_update_n_zero():
    ...
    set_seed(200)
    data = generate_test_data(
        T=3, D_range=(8, 32), rows_range=(10, 50), N=0,
        weight_ty_list=[SPARSE_TYPE_FP16], row_alignment=16,
    )
    run_test(data)
...
def test_single_record():
    ...
    set_seed(201)
    data = generate_test_data(
        T=1, D_range=(4, 4), rows_range=(1, 1), N=1,
        weight_ty_list=[SPARSE_TYPE_INT8], row_alignment=16,
    )
    run_test(data)
...
def test_mixed_placement_skip_non_device():
    ...
    T = 4
    data = generate_test_data(
        T=T, D_range=(8, 32), rows_range=(10, 50), N=30,
        weight_ty_list=[SPARSE_TYPE_FP16], row_alignment=16,
    )
...
def test_large_scale():
    ...
    set_seed(700)
    data = generate_test_data(
        T=32, D_range=(8, 128), rows_range=(100, 1000), N=10000,
        weight_ty_list=[
            SPARSE_TYPE_FP32, SPARSE_TYPE_FP16, SPARSE_TYPE_INT8, SPARSE_TYPE_INT4,
        ],
        row_alignment=16,
    )
    run_test(data)
...
def test_large_D():
    ...
    set_seed(701)
    data = generate_test_data(
        T=2, D_range=(512, 512), rows_range=(10, 10), N=5,
        weight_ty_list=[SPARSE_TYPE_FP32], row_alignment=32,
    )
    run_test(data)
...
def test_large_D_limit(D):
    ...
    set_seed(800 + D)
    data = generate_test_data(
        T=1, D_range=(D, D), rows_range=(2, 2), N=2,
        weight_ty_list=[SPARSE_TYPE_FP32], row_alignment=16,
    )
    run_test(data)
...
COMBO_PARAMS = list(itertools.product(
    COMBO_T_LIST, COMBO_D_RANGES, COMBO_N_LIST, COMBO_ALIGNMENT_LIST
))
# 从全组合中随机采样 100 个用例
random.seed(12345)
...
def test_combo_random_multi_table(T, D_range, N, row_alignment):
    ...
    data = generate_test_data(
        T=T, D_range=D_range, rows_range=(5, 100), N=N,
...
def test_all_types_per_table(T):
    ...
    set_seed(2000 + T)
    data = generate_test_data(
        T=T, D_range=(8, 128), rows_range=(10, 50), N=min(T * 5, 200),
...
def test_single_type_multi_table(weight_ty, T):
    ...
    D_min = 4 if weight_ty in (SPARSE_TYPE_INT2,) else (2 if weight_ty == SPARSE_TYPE_INT4 else 4)
    data = generate_test_data(
        T=T, D_range=(D_min, 64), rows_range=(10, 100), N=min(T * 10, 500),
...
def test_dtype_table_alignment_combo(row_idx_dtype, T, row_alignment):
    ...
    data = generate_test_data(
        T=T, D_range=(8, 128), rows_range=(10, 50), N=min(T * 8, 300),
        weight_ty_list=WEIGHT_TY_LIST,
        row_alignment=row_alignment,
        row_idx_dtype=row_idx_dtype,
    )
...
def test_stress_random_100_rounds(round_idx):
    ...
    data = generate_test_data(
        T=T, D_range=(D_lo, D_hi), rows_range=(2, 50), N=N,
        weight_ty_list=weight_ty_list,
        row_alignment=row_alignment,
