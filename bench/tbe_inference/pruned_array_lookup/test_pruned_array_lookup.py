...
def test_pruned_array_lookup(
    ...
    _set_seed()
    logging.info("table_num: %d, batch_num: %d, length: %d, param_types: %s", table_num, batch_num, length, param_types)
    ...
    for t in range(table_num):
        ...
        dense_indices_t = (dense_indices.view(table_num, batch_num, length))[t].view(-1).to(current_device)
...
def test_diff_batch_per_table():
    ...
    with pytest.raises(RuntimeError):
        ...
        indices, dense_indices, offsets, index_array, index_array_offsets = [
            data.to(current_device) for data in data_list
        ]
