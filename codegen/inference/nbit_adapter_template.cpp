...
constexpr int64_t kInt8QparamsBytes = 8;
...
at::Tensor int_nbit_split_embedding_codegen_lookup_function_npu(
    at::Tensor dev_weights, at::Tensor uvm_weights, at::Tensor weights_placements, at::Tensor weights_offsets,
    at::Tensor weights_tys, at::Tensor D_offsets, const int64_t total_D, const int64_t max_int2_D,
    const int64_t max_int4_D, const int64_t max_int8_D, const int64_t max_float16_D, const int64_t max_float32_D,
    at::Tensor indices, at::Tensor offsets, const int64_t pooling_mode, std::optional<at::Tensor> indice_weights,
    const int64_t output_dtype, std::optional<at::Tensor> lxu_cache_weights,
    std::optional<at::Tensor> lxu_cache_locations, const std::optional<int64_t> row_alignment,
    const std::optional<int64_t> max_float8_D, const std::optional<int64_t> fp8_exponent_bits,
    const std::optional<int64_t> fp8_exponent_bias)
{
    ...
    std::vector<int64_t> max_D_list{max_int2_D, max_int4_D, max_int8_D, maxFloat8DValue, max_float16_D, max_float32_D};
    ...
    // 3. 基础检查
    TORCH_CHECK(featCnt > 0, "weights_offsets size must be greater than 0.");
    TORCH_CHECK(offsets_processed.size(0) > 1, "offsets dim_0 must be greater than 1.");
    ...
    if (static_cast<PoolingMode>(pooling_mode) == PoolingMode::NONE) {
        ...
        int64_t outputCols = max_D;
        if (static_cast<SparseType>(output_dtype) == SparseType::INT8) {
            outputCols += kInt8QparamsBytes;
        }
        Tensor output = at::zeros({totalLen, outputCols}, dev_weights.options().dtype(output_scalar_type));
        if (totalLen == 0) {
            return output;
        }
    ...
    at::ScalarType output_scalar_type = sparse_type_to_scalar_type(output_dtype);
    int64_t outputCols = total_D;
    if (static_cast<SparseType>(output_dtype) == SparseType::INT8) {
        outputCols += featCnt * kInt8QparamsBytes;
    }
    Tensor output = at::zeros({batchSize, outputCols}, dev_weights.options().dtype(output_scalar_type));
    if (totalLen == 0) {
        return output;
    }
