...
# 用于 @unittest.skipIf - 根据设备类型选择
npu_unavailable = (not check_npu_available(), "NPU is not available or no NPUs detected")
cuda_unavailable = (not check_cuda_available(), "CUDA is not available or no GPUs detected")

# 根据设备类型确定skip条件
if DEVICE_TYPE == "npu":
    device_unavailable = npu_unavailable
else:
    device_unavailable = cuda_unavailable
...
class LinearizeCacheIndicesTest(unittest.TestCase):
    ...
    @unittest.skipIf(*device_unavailable)
    def test_linearize_cache_indices(self) -> None:
        # 根据设备类型选择设备
        device = "npu" if DEVICE_TYPE == "npu" else "cuda"
        ...
        equal_offsets = torch.tensor([0, 4, 8, 12, 16], dtype=torch.int, device=device)
        varying_offsets = torch.tensor(
            [0, 1, 3, 6, 8, 10, 14, 15, 16], dtype=torch.int, device=device
        )
            ...
            for test_vbe in [False, True]:
                ...
                if test_vbe:
                    ...
                    B_offsets = torch.tensor(
                        [0] + Bs, dtype=torch.int, device=device
                    ).cumsum(0)

                hash_size_cumsum = torch.tensor(hash_size_cumsum_list, device=device)
