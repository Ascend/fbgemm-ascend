...
from common import Net as NetSplit, cpu_and_maybe_npu, INTERN_MODULE, FIXED_EXTERN_API
...
class PooledEmbeddingModulesSplitTest(unittest.TestCase):
    ...
    def test_permutation(self, fwd_only: bool) -> None:
        ...
        input_tensor = torch.Tensor([range(10)]).to(self.device)
        self.assertEqual(
            net.permute_pooled_embeddings(input_tensor).view(10).tolist(),
            [6, 7, 8, 9, 0, 1, 5, 2, 3, 4],
        )
    ...
    def test_permutation_autograd(self) -> None:
        ...
        input_tensor = torch.randn(2, 1).to(self.device)
    ...
    def test_pooled_table_batched_embedding(self) -> None:
        ...
        permute_pooled_embeddings = PermutePooledEmbeddingsSplit(
            embedding_dims,
            permute_order,
            device=self.device,
        )
        ...
        assert torch.allclose(ref_permuted_pooled_emb.to(self.device), permuted_pooled_emb)

    def test_permutation_autograd_meta(self) -> None:
        """
        Test that permute_pooled_embeddings_autograd_split works with meta tensor and
        dynamo export mode
        """
        input_tensor = torch.randn(2, 1)
        net = Net()

        output_cpu = net(input_tensor)
        output_meta = net.to("meta")(input_tensor.to("meta"))

        assert output_meta.shape == output_cpu.shape
        assert input_tensor.shape == output_meta.shape
