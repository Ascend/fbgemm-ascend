...
from common import npu_available, bfloat_quantize, bfloat_dequantize
    ...
    def test_dense_mlp_quantize_ops(
        ...
        if precision == "BF16":
            ...
            if npu_available():
                ...
                dequantized_npu = torch.ops.fbgemm.Bfloat16QuantizedToFloat(
                    quantized_npu
                )
                torch.testing.assert_close(
                    dequantized_npu.cpu(), input_data, rtol=1e-2, atol=1e-2
                )
...
class TestBfloat16QuantizationConversion(unittest.TestCase):
    ...
    def test_quantize_and_dequantize_op(self, nrows: int, ncols: int) -> None:
        ...
        if npu_available():
            ...
            dequantized_data_npu = torch.ops.fbgemm.Bfloat16QuantizedToFloat(
                quantized_data_npu
            )
            torch.testing.assert_close(dequantized_data_npu.cpu(), ref_fp32)
    ...
    @settings(deadline=10000, suppress_health_check=[HealthCheck.filter_too_much])
    def test_quantize_and_dequantize_op_npu_large_nrows_bf16(
        ...
        torch.testing.assert_close(dequantized_data_npu.cpu(), dequantized_data)
