...
def half_floats_to_bytes(floats: np.ndarray) -> np.ndarray:
    ...
    for i, value in enumerate(floats):
        byte_matrix[i] = np.frombuffer(memoryview(value.tobytes()).tobytes(), dtype=np.uint8)
    return byte_matrix
...
def fused_rowwise_nbit_quantize_dequantize_reference(data: np.ndarray, bit: int) -> np.ndarray:
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            unpacked_data[i, j] = (quantized_data[i, j // num_elem_per_byte] >> ((j % num_elem_per_byte) * bit)) & (
                (1 << bit) - 1
            )
...
def fused_rowwise_8bit_quantize_reference(data):
    ...
    quantized_data = np.clip(np.round((data - bias) * inverse_scale), 0, 255)
    ...
    return np.concatenate([quantized_data.astype(np.uint8), scale_bytes, bias_bytes], axis=-1)


def fused_rowwise_8bit_dequantize_reference(fused_quantized):
...
def fused_rowwise_8bit_dequantize_2bytes_padding_scale_bias_first_reference(fused_quantized):
    scale = bytes_to_half_floats(fused_quantized[..., 0:2].astype(np.uint8).reshape(-1, 2))
    scale = scale.reshape(fused_quantized.shape[:-1] + (scale.shape[-1],))
    bias = bytes_to_half_floats(fused_quantized[..., 2:4].astype(np.uint8).reshape(-1, 2))
    bias = bias.reshape(fused_quantized.shape[:-1] + (bias.shape[-1],))
    quantized_data = fused_quantized[..., 4:]
    return quantized_data * scale + bias
