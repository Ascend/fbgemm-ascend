...
static ge::graphStatus ShapeTilingFunc(gert::TilingContext* context, PrunedArrayLookupTilingData& tilingData)
{
    ...
    OPS_LOG_E_IF_NULL("indexRemappingsTensor", context->GetInputTensor(INDEX_REMAPPINGS_INDEX),
    ...
    OPS_CHECK(indicesType != offsetsType,
              OPS_LOG_E("Tiling Debug", "The datatype of indices and offsets must be same."), return ge::GRAPH_FAILED);

    OPS_CHECK(indicesType != ge::DT_INT32 && indicesType != ge::DT_INT64,
              OPS_LOG_E("Tiling Debug", "Invalid data type. indices must only support int64 and int32."),
              return ge::GRAPH_FAILED);

    // 检查维度
    OPS_CHECK(indicesStorageShape.GetDimNum() != INDICES_DIM,
              OPS_LOG_E("Tiling Debug", "Expected dim for indices is %d, but the actual dim is %d.", INDICES_DIM,
                        indicesStorageShape.GetDimNum()),
              return ge::GRAPH_FAILED);
    OPS_CHECK(indexRemappingsStorageShape.GetDimNum() != INDEX_REMAPPINGS_DIM,
              OPS_LOG_E("Tiling Debug", "Expected dim for index_remappings is %d, but the actual dim is %d.",
                        INDEX_REMAPPINGS_DIM, indexRemappingsStorageShape.GetDimNum()),
              return ge::GRAPH_FAILED);
    OPS_CHECK(offsetsStorageShape.GetDimNum() != OFFSETS_DIM,
              OPS_LOG_E("Tiling Debug", "Expected dim for offsets is %d, but the actual dim is %d.", OFFSETS_DIM,
                        offsetsStorageShape.GetDimNum()),
              return ge::GRAPH_FAILED);
    OPS_CHECK(indexRemappingsOffsetsStorageShape.GetDimNum() != INDEX_REMAPPINGS_OFFSETS_DIM,
              OPS_LOG_E("Tiling Debug", "Expected dim for index_remappings_offsets is %d, but the actual dim is %d.",
                        INDEX_REMAPPINGS_OFFSETS_DIM, indexRemappingsOffsetsStorageShape.GetDimNum()),
              return ge::GRAPH_FAILED);

    // 检查长度关系
    OPS_CHECK(indicesLen <= 0,
              OPS_LOG_E("Tiling Debug", "indices length must be greater than 0, got %lld.", indicesLen),
              return ge::GRAPH_FAILED);
    int64_t batchNum = offsetsLen - 1;
    OPS_CHECK(batchNum <= 0, OPS_LOG_E("Tiling Debug", "offsets length must be greater than 0, got %lld.", batchNum),
              return ge::GRAPH_FAILED);
