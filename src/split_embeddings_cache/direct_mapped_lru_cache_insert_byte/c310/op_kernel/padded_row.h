...
// 权重类型枚举：FP32 / FP16 / INT8 / INT4 / INT2 / BF16 / FP8
...
// 计算未对齐的原始行字节数（根据维度 × 类型字节数 + scale/bias）
...
// 计算对齐后的行字节数（向上取整到 rowAlignment 的倍数）
__simt_callee__ inline int32_t PaddedRowSizeBytes(int32_t dim, uint8_t weightTy, int32_t rowAlignment,
                                                  int32_t scaleBiasBytes = kINT8QparamsBytes)
{
