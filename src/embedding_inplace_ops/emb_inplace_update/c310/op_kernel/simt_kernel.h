...
PaddedRowSizeInBytes(int32_t D, uint8_t weight_ty, int32_t row_alignment) {
  int32_t r = 0;
  switch (weight_ty) {
  case SPARSE_TY_FP32:
    r = D * 4;
...
SimtEmbInplaceUpdateMultiThread(
    __gm__ uint8_t *dev_weights, __gm__ uint8_t * /*uvm_weights*/,
    __gm__ int32_t *weights_placements, __gm__ int64_t *weights_offsets,
    __gm__ uint8_t *weights_tys, __gm__ int32_t *D_offsets,
    __gm__ uint8_t *update_weights, __gm__ int32_t *update_table_indices,
    __gm__ RowIdType *update_row_indices, __gm__ int64_t *update_offsets,
    int64_t N, int32_t row_alignment) {
  ...
  int64_t recordStart =
      static_cast<int64_t>(blockIdx) * warpsPerBlock + warpId;
