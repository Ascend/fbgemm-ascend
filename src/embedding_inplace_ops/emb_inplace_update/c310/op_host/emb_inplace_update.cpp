...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    OPS_LOG_E_IF_NULL("update_row_indicesShape", context->GetInputShape(IN_UPDATE_ROW_INDICES),
                      return ge::GRAPH_FAILED);
    OPS_LOG_E_IF_NULL("D_offsetsShape", context->GetInputShape(IN_D_OFFSETS), return ge::GRAPH_FAILED);
    OPS_LOG_E_IF_NULL("update_row_indicesTensor", context->GetInputTensor(IN_UPDATE_ROW_INDICES),
                      return ge::GRAPH_FAILED);
    OPS_LOG_E_IF_NULL("attrs", context->GetAttrs(), return ge::GRAPH_FAILED);

    int64_t totalUpdates = context->GetInputShape(IN_UPDATE_ROW_INDICES)->GetOriginShape().GetShapeSize();
    int64_t dOffsetsLen = context->GetInputShape(IN_D_OFFSETS)->GetOriginShape().GetShapeSize();
    int64_t numTables = dOffsetsLen > 0 ? dOffsetsLen - 1 : 0;

    ge::DataType rowIdxDtype = context->GetInputTensor(IN_UPDATE_ROW_INDICES)->GetDataType();
    OPS_CHECK(rowIdxDtype != ge::DT_INT32 && rowIdxDtype != ge::DT_INT64,
              OPS_LOG_E("[ERROR]EmbInplaceUpdate", "update_row_indices must be int32 or int64."),
              return ge::GRAPH_FAILED);
    int64_t rowIdxIsInt64 = (rowIdxDtype == ge::DT_INT64) ? 1 : 0;

    const int64_t* rowAlignmentAttr = context->GetAttrs()->GetInt(ATTR_ROW_ALIGNMENT);
    OPS_LOG_E_IF_NULL("row_alignment attr", rowAlignmentAttr, return ge::GRAPH_FAILED);
    int64_t rowAlignment = *rowAlignmentAttr;
    OPS_CHECK(rowAlignment < 16 || rowAlignment % 16 != 0,
              OPS_LOG_E("[ERROR]EmbInplaceUpdate", "row_alignment must be a positive multiple of 16 "
    ...
    uint64_t totalWarps = static_cast<uint64_t>(realCoreNum) * warpsPerBlock;
    if (totalUpdates > 0 && static_cast<uint64_t>(totalUpdates) < totalWarps) {
        // N 很小：缩减 warpsPerBlock，避免大量空闲 warp（threadNumPerBlock 仍按 warp 对齐）
        uint32_t neededWarps = static_cast<uint32_t>(std::max<int64_t>(totalUpdates, 1));
    ...
    if (totalUpdates == 0) {
        ...
        uint32_t need = static_cast<uint32_t>((totalUpdates + warpsPerBlock - 1) / warpsPerBlock);
        if (need < gridDim) {
            gridDim = need;
        }
        if (gridDim == 0) {
